...
const ArticleListPage = () => {
  ...
  useEffect(() => {
    const loadArticles = async () => {
      const localImages = [concept1, concept2, concept3, concept4, concept5];

      try {
        const { data } = await fetchArticles();

        const dynamicArticles = data.articles
          ...
          .map((article, index) => {
            ...
            if (imgUrl && imgUrl.startsWith('/uploads')) {
              ...
              imgUrl = localImages[index % localImages.length];
            }
        ...
        setAllArticles(dynamicArticles);
      } catch (error) {
        console.error(error);
        setAllArticles([]);
      }
