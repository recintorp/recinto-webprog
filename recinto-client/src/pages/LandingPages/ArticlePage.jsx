...
function ArticlePage() {
  ...
  const [articleImage, setArticleImage] = useState(concept1);
  ...
  useEffect(() => {
    const loadArticle = async () => {
      ...
      const localIndex = articlesData.findIndex(a => a.name === name);
      
      if (localIndex !== -1) {
        setArticle(articlesData[localIndex]);
        setArticleImage(images[localIndex % images.length]);
        setLoading(false);
        return;
      }
      ...
      try {
        ...
        const dynamicArticle = data.articles.find(a => a.name === name && a.isActive);

        if (dynamicArticle) {
          setArticle(dynamicArticle);
          let imgUrl = dynamicArticle.image || concept1;
          if (typeof imgUrl === 'string' && imgUrl.startsWith('/uploads')) {
            imgUrl = `${constants.IMAGE_HOST}${imgUrl}`;
          }
          setArticleImage(imgUrl);
        } else {
          setArticle(null);
        }
      } catch (error) {
  ...
  if (loading) {
    return (
      <div className="flex w-full min-h-screen items-center justify-center bg-[#030108]">
        <div className="w-12 h-12 border-2 border-white/10 border-t-[#C084FC] rounded-full animate-spin"></div>
      </div>
    );
  }
